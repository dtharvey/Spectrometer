...
server = function(input, output) {
  ...
  output$standard1 = renderPlot({
    plot(x = std$wavelength, y = std$absorbance,
         lwd = 2, col = "magenta", type = "l",
         xlab = "wavelength (nm)", xlim = c(240,290),
         ylab = "absorbance (arbitrary units)", ylim = c(-0.1,0.6),
         main = "standard spectrum"
         )
    abline(v = input$wavelength, lty = 2, col = "black")
    abline(h = input$absorbance, lty = 2, col = "black")
    grid()
  })
  ...
  output$questionspectra = renderPlot({
    if (input$questionnumber == "Set 1"){
      ...
      grid()
      par(old.par)
    } else if (input$questionnumber == "Set 2"){
      old.par = par(mfrow = c(2,2))
      ...
      grid()
      par(old.par)
    } else if (input$questionnumber == "Set 3"){
      old.par = par(mfrow = c(2,2))
